        ...
        >
          ...
          <div class="flex items-center gap-2 w-full sm:w-auto mt-1 sm:mt-0 pl-9 sm:pl-0 sm:ml-auto">
            ...
            <div class="flex-1 sm:flex-initial min-w-0">
               <template v-if="editingTodoId !== todo.id">
                 <div @dblclick="startEditing(todo)" v-if="todo.start_date || todo.due_date" class="text-[10px] sm:text-[11px] tracking-wide flex items-center gap-1 cursor-pointer whitespace-nowrap" :class="getDateClass(todo)">
               ...
               <div v-else class="flex items-center gap-1">
                  <input v-model="tempTodoStartDate" type="date" class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/5 rounded-lg px-1 py-0.5 text-[10px] outline-none [color-scheme:dark] w-20 sm:w-24" />
                  <span class="text-slate-400">-</span>
                  <input v-model="tempTodoDueDate" type="date" class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/5 rounded-lg px-1 py-0.5 text-[10px] outline-none [color-scheme:dark] w-20 sm:w-24" />
               </div>
            </div>
            ...
            <div class="flex items-center gap-0.5 sm:gap-1">
              ...
              <div class="flex items-center sm:opacity-0 group-hover/item:opacity-100 transition-opacity gap-0.5 sm:gap-1">
                <!-- Edit Actions -->
                <template v-if="editingTodoId === todo.id">
                  <button @click="saveEditing(todo)" class="p-1.5 text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg transition-all" title="保存">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                  </button>
                  <button @click="cancelEditing" class="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all" title="取消">
                ...
                <!-- Regular Actions -->
                <template v-else>
                  <button v-if="todo.status === 'pending'" @click="startEditing(todo)" class="p-1.5 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-lg transition-all" title="编辑内部">
                  ...
                  <div v-if="todo.status === 'pending'" class="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg overflow-hidden p-0.5">
                    <button @click="gadgetStore.postponeTodo(todo.id, 1)" class="px-1.5 py-1 text-[9px] font-black text-slate-500 hover:text-primary hover:bg-white dark:hover:bg-slate-700 rounded-md transition-all">1D</button>
                    <button @click="gadgetStore.postponeTodo(todo.id, 3)" class="px-1.5 py-1 text-[9px] font-black text-slate-500 hover:text-primary hover:bg-white dark:hover:bg-slate-700 rounded-md transition-all border-l border-slate-200 dark:border-white/5">3D</button>
                    <button @click="gadgetStore.postponeTodo(todo.id, 7)" class="px-1.5 py-1 text-[9px] font-black text-slate-500 hover:text-primary hover:bg-white dark:hover:bg-slate-700 rounded-md transition-all border-l border-slate-200 dark:border-white/5">1W</button>
                  </div>

                  <button v-if="todo.status === 'pending'" @click="gadgetStore.failTodo(todo.id)" class="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="标记为失败">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                  </button>

                  <button @click="gadgetStore.removeTodo(todo.id)" class="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all" title="删除任务">
