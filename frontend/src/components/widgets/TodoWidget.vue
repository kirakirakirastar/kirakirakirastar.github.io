    ...
    <div class="flex-1 overflow-y-auto space-y-3 custom-scrollbar max-h-[400px] pr-1">
      ...
      <transition-group v-else name="list-complete" tag="div" class="space-y-3">
        <TodoItem 
          ...
          @toggle-status="(child) => toggleStatus(child || todo)"
          @cycle-priority="(child) => cyclePriority(child || todo)"
          @cycle-recurrence="(child) => cycleRecurrence(child || todo)"
          @postpone="(days, child) => gadgetStore.postponeTodo((child || todo).id, days)"
          @fail="(child) => gadgetStore.failTodo((child || todo).id)"
          @retry="(child) => gadgetStore.updateTodoStatus((child || todo).id, 'pending')"
          @remove="(child) => gadgetStore.removeTodo((child || todo).id)"
          @start-edit="(id) => editingTodoId = id"
          @cancel-edit="editingTodoId = null"
          @save-edit="(updates, child) => handleSaveEdit((child || todo).id, updates)"
