    ...
    <div class="flex-1 overflow-y-auto space-y-3 custom-scrollbar max-h-[300px] pr-1">
      ...
      <div v-else-if="activeTodos.length === 0" class="text-center py-10 text-slate-400 dark:text-slate-500">
        ...
        >
          ...
          <!-- Main Content (Checkbox + Text/Edit) -->
          ...
          <div class="flex items-center gap-1 w-full sm:w-auto justify-end sm:ml-auto">
            ...
            <button 
              v-else
              @click="cycleRecurrence(todo)"
              class="flex-shrink-0 p-1.5 text-slate-300 dark:text-slate-600 sm:opacity-0 group-hover/item:opacity-100 hover:text-primary hover:bg-primary/5 rounded-lg transition-all"
              title="设置循环"
            >
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
            ...
            <div class="flex items-center sm:opacity-0 group-hover/item:opacity-100 transition-opacity gap-1">
        ...
        </div>01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                </svg>
              </button>
            </template>
          </div>
